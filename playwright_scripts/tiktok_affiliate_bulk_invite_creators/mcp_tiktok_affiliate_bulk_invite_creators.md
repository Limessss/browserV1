...
1. 等待"查找达人"页面加载。
2. 默认应用筛选：`商品类目=女装与女士内衣`、`平均佣金率=小于 15%`、`过去 90 天内未获邀请的达人`；如果传入其他筛选参数，继续应用对应筛选。
3. 勾选 1 位达人并打开 `批量邀请` 弹窗，识别弹窗结构。
4. 切换到 `创建新邀请` 标签。
5. 选择 `仅佣金` 类型。
6. 点击弹窗底部 `邀请` 按钮，跳转到表单页 `/target-invitation/create`。
7. 填写表单：
   - 邀请名称（默认日期+4位随机字符串）
   - 有效期截止（默认一年后）
   - Facebook 账号（默认 `linkeoo`）
   - 邀请文本（默认英文模板）
8. 展开"选择商品"面板，点击 `添加商品` 打开弹窗：
   - 切换搜索字段为 `商品 ID`
   - 输入并搜索用户提供的商品ID
   - 点击搜索图标
   - 勾选搜索结果中的商品

...
/**
 * v20 探针：完整 dump "绑定现有商品" 抽屉 DOM，找真实 checkbox / 勾选 / 添加按钮位置。
 * 不勾不提交，只观察。
 */
...
function logStep(msg) { process.stdout.write(`[v20 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'drawer_dom_v20_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', drawerResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    const wrapped = `(() => { window.__PENDING_KEYWORD__ = ${JSON.stringify(keyword)}; return (${scriptText})(); })()`
    result.drawerResult = await page.evaluate(wrapped)
    result.ok = !!result.drawerResult
    logStep(`drawerResult.ok=${result.drawerResult?.ok}, rounds=${(result.drawerResult?.rounds || []).length}`)
    for (const r of result.drawerResult?.rounds || []) {
      const cbs = (r.checkboxes || []).length
      const tps = (r.targetPidMatches || []).length
      logStep(`phase=${r.phase}: inputs=${(r.inputs || []).length} checkboxes=${cbs} pid-matches=${tps} buttons=${(r.buttons || []).length}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_drawer_v20_${stamp}.json`)

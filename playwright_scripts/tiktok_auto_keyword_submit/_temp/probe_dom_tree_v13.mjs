...
/**
 * v13 探针：在 trending_keywords 真实页面，dump .core-table-body 完整子树 + 查找所有可点击提报入口。
 * 期望找出真实可走 DOM 提报的"行 trigger"或"提报按钮"。
 */
...
function logStep(msg) { process.stdout.write(`[v13 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'dom_tree_script.js'), 'utf8')
  ...
  try {
    ...
    await sleep(waitMs)
    logStep('running dom tree discovery')
    result.discovery = await page.evaluate(`(${scriptText})()`)
    result.ok = !!result.discovery
    logStep(`targetElCount=${result.discovery?.targetElCount}`)
  } catch (e) {
    result.errors.push(e instanceof Error ? e.message : String(e))
    logStep(`FAILED: ${result.errors[0]}`)
  } finally {
    const stamp = new Date().toISOString().replace(/[:.]/g, '-')
    const jsonPath = path.join(outDir, `probe_dom_tree_v13_${stamp}.json`)

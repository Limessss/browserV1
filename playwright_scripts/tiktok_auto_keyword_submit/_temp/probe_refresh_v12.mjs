...
/**
 * v12 探针：刷新 + 等长（25s），看 tr.core-table-tr 是否真能补齐。
 */
...
function logStep(msg) { process.stdout.write(`[v12 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const waitMs = Number(getArgValue('--wait_ms') || 5_000)
  const scriptDir = path.dirname(fileURLToPath(import.meta.url))
  const outDir = getArgValue('--out_dir') || path.join(scriptDir, 'debug_reports')
  await mkdir(outDir, { recursive: true })

  const scriptText = await readFile(path.join(scriptDir, 'refresh_wait_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', refreshResult: null, errors: [] }
  try {
    logStep('navigating')
    await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: navTimeoutMs })
    logStep(`domcontentloaded, current URL: ${page.url()}`)
    logStep(`waiting ${waitMs}ms then run refresh+wait`)
    await sleep(waitMs)
    logStep('running refresh + wait')
    // 注意 location.reload() 会让 evaluate 中断—— Playwright 仍会重新建立连接
    try {
      result.refreshResult = await page.evaluate(`(${scriptText})()`)
    } catch (e) {
      logStep(`evaluate failed (likely page reload race): ${e instanceof Error ? e.message : String(e)}`)
      // 重新等加载
      ...
      result.refreshResult = {
        rounds: [{
          phase: 'post-error-fallback',
          trCount: await page.locator('tr.core-table-tr, .core-table-body tr, .core-table-content-inner tr').count(),
          markbuttonCount: await page.locator('[data-uid^="markbutton"]').count(),
          bodyTail: (await page.evaluate(() => document.body?.innerText || '')).slice(-600),
        }],
      }
    }
    result.ok = !!result.refreshResult
    for (const r of result.refreshResult.rounds) {
      logStep(`phase=${r.phase}: trCount=${r.trCount}, markbuttonCount=${r.markbuttonCount}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_refresh_v12_${stamp}.json`)

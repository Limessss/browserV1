...
/**
 * v11 探针：尝试多种滚动策略触发虚拟列表补齐 tr.core-table-tr。
 */
...
function logStep(msg) { process.stdout.write(`[v11 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'scroll_strategy_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', scrollResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    logStep('running scroll strategies')
    result.scrollResult = await page.evaluate(`(${scriptText})()`)
    result.ok = true
    for (const r of result.scrollResult.rounds) {
      logStep(`phase=${r.phase}: trCount=${r.snap.trCount}, markbuttonCount=${r.snap.markbuttonCount}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_scroll_v11_${stamp}.json`)

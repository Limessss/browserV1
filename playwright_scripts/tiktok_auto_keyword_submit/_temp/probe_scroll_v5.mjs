...
/**
 * TikTok 自动关键词提报 - 滚动分页解析 v5 探针。
 *
 * v4 发现：.core-table-body.innerText 在虚拟滚动下只取视口内 ~12 行；
 * 全部 ~13000px / 几十页的"真实"内容无法一次拿到。
 *
 * v5 多次滚动（每次 scrollTop += clientHeight - 50），每段 dump innerText，
 * 合并去重所有行，按 source 计数并列出 source=「搜索关键词」的样本。
 *
 * 用法：
 *   node playwright_scripts/tiktok_auto_keyword_submit/_temp/probe_scroll_v5.mjs \
...
function logStep(msg) { process.stdout.write(`[scrollv5 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const parseScriptText = await readFile(path.join(scriptDir, 'parse_scroll_script.js'), 'utf8')
  ...
  try {
    ...
    await sleep(waitMs)
    logStep(`running scroll parser (${parseScriptText.length} bytes)`)
    result.parsed = await page.evaluate(`(${parseScriptText})()`)
    result.ok = (result.parsed?.searchKeywordCount || 0) > 0
    logStep(`searchKeywordCount=${result.parsed?.searchKeywordCount || 0}, totalUniqueRows=${result.parsed?.totalUniqueRows || 0}`)
    logStep(`bySource=${JSON.stringify(result.parsed?.bySource || {})}`)
  } catch (e) {
    result.errors.push(e instanceof Error ? e.message : String(e))
    logStep(`FAILED: ${result.errors[0]}`)
  } finally {
    const stamp = new Date().toISOString().replace(/[:.]/g, '-')
    const jsonPath = path.join(outDir, `probe_scroll_v5_${stamp}.json`)

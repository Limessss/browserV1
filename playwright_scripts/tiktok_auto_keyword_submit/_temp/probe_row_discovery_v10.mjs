...
/**
 * 探针 v10：在 trending_keywords 页面真实查找"包含 lead 文本"的可见行容器。
 * v9 探针证实 tr.core-table-tr 永远只 1 行（虚拟表头），但 lead 文本真实存在；
 * v10 尝试找出真正承载行的 DOM 节点（不限于 tr），含 data-row-key、markbutton 等。
 */
...
function logStep(msg) { process.stdout.write(`[rowdisc ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'row_discovery_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', discovery: null, errors: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    logStep('running row discovery')
    result.discovery = await page.evaluate(`(${scriptText})()`)
    result.ok = !!result.discovery
    logStep(`trCount=${result.discovery?.trCount}, markbuttonCount=${result.discovery?.markbuttonCount}`)
  } catch (e) {
    result.errors.push(e instanceof Error ? e.message : String(e))
    logStep(`FAILED: ${result.errors[0]}`)
  } finally {
    const stamp = new Date().toISOString().replace(/[:.]/g, '-')
    const jsonPath = path.join(outDir, `probe_row_discovery_v10_${stamp}.json`)

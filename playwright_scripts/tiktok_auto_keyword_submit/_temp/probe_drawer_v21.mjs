...
/**
 * v21 探针：搜索 productId 后，依次尝试：
 *   - 整行 click
 *   - 行内"选择"/"Select"按钮 click
 * dump 每步的"已选 N 件商品" + 全部按钮文本 + bodyTail，找真实"添加商品"动作。
 */
...
function logStep(msg) { process.stdout.write(`[v21 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'drawer_v21_script.js'), 'utf8')
  ...
  try {
    ...
    for (const r of result.drawerResult?.rounds || []) {
      logStep(`phase=${r.phase}: selected="${(r.selectedText || '').slice(0, 40)}" buttons=${(r.buttons || []).length}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_drawer_v21_${stamp}.json`)

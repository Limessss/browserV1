...
/**
 * v19 探针：循环 3 个 productId search+勾选，**每步 dump 抽屉"已选 N 件商品"**计数。
 * 验证多商品是否真的一次性勾选成功。
 * ⚠️ 不点"提交"，避免污染账号。
 */
...
function logStep(msg) { process.stdout.write(`[v19 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'multi_select_v19_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', keyword: '', productIds, multiResult: null, errors: [], discoveredLeads: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    // 自动从当前页面真实可见的 div.core-table-tr 行里抽 lead_name
    ...
    const keyword = getArgValue('--keyword') || discovered[0] || ''
    if (!keyword) throw new Error('页面找不到任何 lead 行')
    logStep(`discovered ${discovered.length} leads, using keyword="${keyword}"`)
    result.keyword = keyword
    result.discoveredLeads = discovered
    logStep(`⚠️ 真实多商品 search+勾选，dump 每步"已选 N 件商品"`)
    const wrapped = `(() => { window.__PENDING_KEYWORD__ = ${JSON.stringify(keyword)}; return (${scriptText})(${JSON.stringify(productIds)}); })()`
    result.multiResult = await page.evaluate(wrapped)
    result.ok = !!result.multiResult
    for (const r of result.multiResult.steps || []) {
      logStep(`step=${r.step}: selected=${JSON.stringify(r.selected)} rowText="${(r.rowText || '').slice(0, 50)}" checkedAfterClick=${r.checkedAfterClick}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_multi_v19_${stamp}.json`)

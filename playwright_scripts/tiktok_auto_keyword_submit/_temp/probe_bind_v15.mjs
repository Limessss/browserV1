...
/**
 * v15 探针：点 div.core-table-tr → 弹出 drawer → 点 "绑定现有商品" → dump 抽屉内结构。
 */
...
function logStep(msg) { process.stdout.write(`[v15 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'bind_drawer_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', bindResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    logStep('running bind drawer probe')
    result.bindResult = await page.evaluate(`(${scriptText})()`)
    result.ok = !!result.bindResult
    for (const r of result.bindResult.rounds || []) {
      logStep(`phase=${r.phase}: btnText="${r.btnText || ''}" inputs=${(r.inputPlaceholders || []).length} bodyTailLen=${(r.bodyTail || '').length}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_bind_v15_${stamp}.json`)

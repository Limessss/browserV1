...
/**
 * v22 探针：单 lead + 2 productIds，每个 productId 独立走"开 drawer→搜→选→下一步→提交"。
 * ⚠️ 真实提交 2 次。
 */
...
function logStep(msg) { process.stdout.write(`[v22 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  // 真实 productId：v15 探针真实成功过的（属 "Cotton Spandex Cross Over Blouse" lead）
  const productIds = [
    '1735873998701954740', // Thin Women's Shirt Summer ...
  ]
  const scriptText = await readFile(path.join(scriptDir, 'single_product_loop_v22_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', productIds, loopResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    const discovered = await page.evaluate(() => {
      ...
      for (const tr of rows) {
        ...
        // 排除已提报过的（带"已提报"或"审核中"标志的行）
        if (text.includes('已提报') || text.includes('审核中')) continue;
        const m = text.match(/^#\s*([^\n]+?)(?=Womenswear|Menswear|全球|$)/);
        if (m) out.push(m[1].trim());
      }
      return out.slice(0, 30);
    });
    // 真实 lead：v17 真实跑通过的 "Cotton Spandex Cross Over Blouse"——但它已被提报过，会被 filter 排除。
    // 选 discovered[0]（自动是当前真实可见的未提报 lead）
    const keyword = discovered[0] || getArgValue('--keyword') || ''
    if (!keyword) throw new Error('discovered 为空且未传 --keyword')
    logStep(`discovered ${discovered.length} leads, using keyword="${keyword}"`)
    const wrapped = `(() => { window.__PENDING_KEYWORD__ = ${JSON.stringify(keyword)}; return (${scriptText})(${JSON.stringify(productIds)}); })()`
    result.loopResult = await page.evaluate(wrapped)
    result.ok = !!result.loopResult
    for (const r of result.loopResult.rounds || []) {
      logStep(`phase=${r.phase}: selected="${(r.selectedText || '').slice(0, 30)}" success=${r.success || ''} innerBtn="${(r.innerBtnText || '').slice(0, 30)}"`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_loop_v22_${stamp}.json`)

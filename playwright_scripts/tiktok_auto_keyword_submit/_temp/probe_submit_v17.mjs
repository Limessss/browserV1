...
function logStep(msg) { process.stdout.write(`[v17 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'submit_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', submitResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    logStep('⚠️ 真实提交到 TikTok 机会系统')
    result.submitResult = await page.evaluate(`(${scriptText})()`)
    result.ok = !!result.submitResult
    for (const r of result.submitResult.rounds || []) {
      logStep(`phase=${r.phase}: ${r.error || r.note || ''}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_submit_v17_${stamp}.json`)

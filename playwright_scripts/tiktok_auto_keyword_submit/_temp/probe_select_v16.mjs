...
/**
 * v16 探针：完整真实 DOM 提报流程 —— 行 click → 绑定现有商品 → 搜索 productId → 勾选 → 下一步 → dump。
 */
...
function logStep(msg) { process.stdout.write(`[v16 ${new Date().toLocaleTimeString()}] ${msg}\n`) }
...
async function main() {
  ...
  const scriptText = await readFile(path.join(scriptDir, 'select_next_script.js'), 'utf8')
  ...
  const result = { code: getArgValue('--code') || 'GMNQ5O', ok: false, targetUrl, cdpUrl: conn.cdpUrl || '', selectResult: null, errors: [] }
  try {
    logStep('navigating')
    ...
    await sleep(waitMs)
    logStep('running select+next probe')
    result.selectResult = await page.evaluate(`(${scriptText})()`)
    result.ok = !!result.selectResult
    for (const r of result.selectResult.rounds || []) {
      logStep(`phase=${r.phase}: ${r.error || ''}`)
    }
  } catch (e) {
    ...
    const jsonPath = path.join(outDir, `probe_select_v16_${stamp}.json`)

...
- 如果 `商品名称` 为空，按 `shop_region` 写入“视频中的商品”的内置翻译，例如 TH 写入 `สินค้าในวิดีโอ`。
- 滚动到 `AI 生成的内容`，确保开关处于开启状态。
6. 如果过早点最终发布，会出现 `将商品添加到你的视频` 对话框。脚本会关闭该对话框，等待商品卡片加载后重试。
...
- 脚本会补齐空的 `商品名称`，再打开 `AI 生成的内容` 开关后最终发布。

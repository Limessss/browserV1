...
function useWailsNotifications() {
  ...
  useEffect(() => {
    ...
    const offBridgeDied = runtime.EventsOn(
      'proxy:bridge:died',
      (data: { key: string; engine?: string; code?: number | null; signal?: string | null }) => {
        const tail = data?.signal
          ? `（signal: ${data.signal}）`
        ...
        addNotification({
          type: 'warning',
          title: '连接池节点失效',
          message: `代理节点 ${data?.key || '-'}${data?.engine ? ` [${data.engine}]` : ''} 连接中断${tail}，相关实例可能无法访问网络`,
        })
      }
    )

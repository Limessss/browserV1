...
export function BrowserEditPage() {
  ...
  const [defaultStartUrlsText, setDefaultStartUrlsText] = useState('')
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const normalizedCoreId = !current.coreId || current.coreId.toLowerCase() === 'default'
        ? ''
        : current.coreId
      const dsu = current.defaultStartUrls?.filter(Boolean) ?? []
      setFormData({
        ...
        defaultStartUrls: dsu,
      })
      setLaunchArgsText(currentLaunchArgs.join('\n'))
      setDefaultStartUrlsText(dsu.join('\n'))
    }
    loadData()
  }, [id, isCreate])
  ...
  const handleSave = async () => {
    ...
    const payload: BrowserProfileInput = {
      ...
      defaultStartUrls: normalizeLaunchArgs(defaultStartUrlsText.split('\n')),
    }
    try {
      if (isCreate) {

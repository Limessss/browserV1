...
function formatIPHealthError(result: ProxyIPHealthResult, fallbackName: string): string {
  const code = String(result.errorCode ?? '').trim()
  if (!code) return result.error || `${fallbackName} 检测失败`
  if (code === 'NO_PROXY') return '未找到代理配置，请先检查代理池节点是否存在'
  if (code === 'DIRECT_UNSUPPORTED') return '直连模式不支持出口 IP 健康检测'
  if (code === 'BRIDGE_FAILED') return result.error || '代理桥接启动失败，请检查 xray/sing-box 与节点配置'
  if (code === 'REQUEST_TIMEOUT') return 'IP 健康检测请求超时，请稍后重试'
  if (code === 'RESPONSE_INVALID') return 'IP 健康检测响应无效，请稍后重试'
  if (code === 'REQUEST_FAILED') return result.error || 'IP 健康检测请求失败'
  return result.error || `${fallbackName} 检测失败`
}
...
export function ProxyPoolPage() {
  ...
  const handleCheckOneIPHealth = async (record: ProxyDisplayInfo) => {
    ...
    try {
      ...
      if (!result.ok) {
        toast.error(formatIPHealthError(result, record.proxyName))
      }
    } finally {
  ...
  const renderIPHealth = (record: ProxyDisplayInfo) => {
    ...
    if (!result.ok) {
      const errText = formatIPHealthError(result, record.proxyName)
      return (
        <div className="flex items-center gap-2">
          <span className="text-xs text-red-500 truncate max-w-[120px]" title={errText}>失败</span>
